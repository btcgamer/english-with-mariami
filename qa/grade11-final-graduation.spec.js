...
test('Grade 11 final graduation — all 10 worlds and 200 missions', async ({ page }) => {
  const pageErrors = [];
...
test('Grade 11 graduation progress persists after reload', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('magicCurriculumProgress:g11', JSON.stringify(Array.from({ length: 200 }, (_, i) => {
      const world = Math.floor(i / 20) + 1;
      const mission = (i % 20) + 1;
      return `11:${world}:${mission}`;
    })));
  });
  await page.goto(`${BASE}/advanced-academy.html?grade=11&world=10`, { waitUntil: 'domcontentloaded' });
  await waitForRuntime(page);
  await expect(page.locator('[data-curriculum-total-progress]')).toContainText('200/200');
