...
async function restore(u){const r=await client.from('student_progress').select('words_learned,quiz_completed,best_quiz').eq('user_id',u.id).eq('grade',2).maybeSingle();if(r.error||!r.data)return;const p=read();const has=Array.isArray(p.done)&&p.done.length||Number(p.xp)||Number(p.stars);if(!has&&(Number(r.data.words_learned)||Number(r.data.quiz_completed))){const n=Math.min(12,Math.round(Number(r.data.words_learned||0)/25));p.done=Array.from({length:n},(_,i)=>i);p.xp=Number(r.data.words_learned||0);p.stars=Number(r.data.quiz_completed||0);write(p);location.reload()}}
async function save(u){const p=read(),done=Array.isArray(p.done)?p.done:[];const words=Math.min(300,done.length*25+Math.min(25,Number(p.xp||0)%100));const quiz=Math.max(0,Number(p.stars||0));const best=done.length===12?100:Math.min(100,Number(p.xp||0)%100);const now=new Date().toISOString();const r=await client.from('student_progress').upsert({user_id:u.id,grade:2,words_learned:words,quiz_completed:quiz,score:best,learned_words:done.map(i=>'grade2-mission-'+(Number(i)+1)),best_quiz:best,last_active_at:now,updated_at:now},{onConflict:'user_id,grade'});badge(r.error?'⚠ Local progress safe':'✓ Supabase synced')}
