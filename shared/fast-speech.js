...
(function(){'use strict';
  ...
  const pickVoice=(lang)=>{
    ...
    return voices.find(v=>v.lang&&v.lang.toLowerCase()===wanted)||voices.find(v=>v.lang&&v.lang.toLowerCase().startsWith(wanted.slice(0,2)))||voices.find(v=>v.lang&&v.lang.toLowerCase().startsWith('en'))||voices[0]||null;
  };
  const stop=()=>{if(!supported)return;clearTimeout(speakTimer);try{synth.cancel();}catch(e){}active=null;};
  ...
  const renderDiagnostic=(r,status)=>{
    const box=document.getElementById('magic-speech-diagnostic');if(!box)return;
    box.querySelector('[data-sd-status]').textContent=status;
    box.querySelector('[data-sd-body]').textContent='API: '+(r.supported?'SUPPORTED':'NOT SUPPORTED')+'\nVoices: '+r.voices+' (English: '+r.englishVoices+')\nVoice: '+(r.selectedVoice||'NONE')+(r.selectedLang?' ['+r.selectedLang+']':'')+'\nStarted: '+r.started+' | Ended: '+r.ended+'\nSpeaking: '+r.speaking+' | Pending: '+r.pending+' | Paused: '+r.paused+'\nError: '+(r.error||'NONE')+'\nDevice: '+r.userAgent;
  };
  const diagnostic=()=>{
    refreshVoices();const started=Date.now();const english=voices.filter(v=>/^en(?:-|$)/i.test(v.lang||''));
    const report={supported,voices:voices.length,englishVoices:english.length,selectedVoice:(pickVoice('en-US')||{}).name||null,selectedLang:(pickVoice('en-US')||{}).lang||null,speaking:!!synth.speaking,pending:!!synth.pending,paused:!!synth.paused,started:false,ended:false,error:null,elapsed:null,userAgent:navigator.userAgent};
    const u=new SpeechSynthesisUtterance('Hello. This is the English with Mariami audio test.');u.lang='en-US';u.rate=.9;const v=pickVoice('en-US');if(v)u.voice=v;
    ...
    try{synth.cancel();synth.resume();synth.speak(u);renderDiagnostic(report,'SENT — waiting for STARTED / ERROR');}catch(e){report.error=e&&e.message?e.message:String(e);renderDiagnostic(report,'ERROR — '+report.error);}
  };
  const installDiagnostic=()=>{
    if(document.getElementById('magic-speech-diagnostic'))return;
    const box=document.createElement('section');box.id='magic-speech-diagnostic';box.style.cssText='position:fixed;left:12px;right:12px;bottom:12px;z-index:2147483647;padding:14px;border:1px solid rgba(80,220,255,.65);border-radius:14px;background:rgba(5,10,25,.96);color:#dffaff;font:13px/1.45 system-ui,sans-serif;box-shadow:0 0 24px rgba(0,220,255,.22);backdrop-filter:blur(10px);display:none;';
    ...
    document.body.appendChild(box);box.querySelector('[data-sd-close]').onclick=()=>box.style.display='none';box.querySelector('[data-sd-test]').onclick=diagnostic;
    const launcher=document.createElement('button');launcher.type='button';launcher.id='magic-speech-launcher';launcher.textContent='🔊 Audio Test';launcher.style.cssText='position:fixed;right:12px;bottom:12px;z-index:2147483646;padding:9px 12px;border:1px solid rgba(80,220,255,.65);border-radius:999px;background:rgba(5,10,25,.94);color:#dffaff;font:600 13px system-ui,sans-serif;box-shadow:0 0 16px rgba(0,220,255,.18);';launcher.onclick=()=>{box.style.display='block';renderDiagnostic({supported,voices:voices.length,englishVoices:voices.filter(v=>/^en(?:-|$)/i.test(v.lang||'')).length,selectedVoice:(pickVoice('en-US')||{}).name||null,selectedLang:(pickVoice('en-US')||{}).lang||null,speaking:!!synth.speaking,pending:!!synth.pending,paused:!!synth.paused,started:false,ended:false,error:null,userAgent:navigator.userAgent},'READY — tap TEST AUDIO');};document.body.appendChild(launcher);
  };
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',installDiagnostic,{once:true});else installDiagnostic();
})();