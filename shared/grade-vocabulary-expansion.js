/* English with Mariami — large vocabulary expansion for Grades 2–4.
...
function mount(){const grade=Number(document.body?.dataset.grade);if(!DATA[grade])return;const main=document.querySelector('main');if(!main||main.querySelector('.ewm-vocab-expansion'))return;styles();const groups=DATA[grade];const words=groups.flatMap(g=>g[1]);const unique=[...new Set(words)];const active=groups.find(g=>{const text=(document.querySelector('.title')?.textContent||'').toLowerCase();return text===g[0].toLowerCase()})||groups[0];const vocab=active[1];const box=document.createElement('section');box.className='ewm-vocab-expansion';box.innerHTML=`<div class="ewm-vocab-head"><div><h2>📚 English Word Vault</h2><div class="ewm-vocab-sub">${unique.length} extra words across Grade ${grade} • active world: ${active[0]}</div></div><button class="btn ewm-say-all" type="button">🔊 Listen to words</button></div><div class="ewm-word-grid">${vocab.map(w=>`<button class="ewm-word" type="button" data-word="${w.replace(/"/g,'&quot;')}"><span>${w}</span><small>🔊</small></button>`).join('')}</div><h3 style="margin:22px 0 0">💬 Useful English Phrases</h3><div class="ewm-phrase-grid">${PHRASES[grade].map(p=>`<button class="ewm-word ewm-phrase" type="button" data-phrase="${p.replace(/"/g,'&quot;')}">${p}<small>🔊</small></button>`).join('')}</div><div class="ewm-vocab-more">Keep reading, speaking, listening, and using new words in complete sentences.</div>`;main.appendChild(box);box.addEventListener('click',e=>{const b=e.target.closest('[data-word],[data-phrase],.ewm-say-all');if(!b)return;if(b.classList.contains('ewm-say-all'))speak([...vocab,...PHRASES[grade]].join('. '));else speak(b.dataset.word||b.dataset.phrase)})}
function watch(){mount();const mo=new MutationObserver(()=>{clearTimeout(window.__ewmVocabTimer);window.__ewmVocabTimer=setTimeout(mount,40)});mo.observe(document.body,{childList:true,subtree:true})}
