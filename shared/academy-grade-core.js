...
async function loadDbLesson(n){
  ...
  try{
    ...
    console.warn('Grade '+grade+' DB lesson load skipped:',error);
    return null;
  }
}
...
const G1Topics=['Alphabet & Greetings','Numbers','Colors & Shapes','Family & People','My Body','Animals','Birds & Nature','School','Home','Food & Drinks','Weather & Seasons','About Me & Review'];
const G1Words=['apple, ball, cat, dog, egg, fish, hello, goodbye','one, two, three, four, five, six, seven, eight','red, blue, green, yellow, circle, square, star, heart','mother, father, sister, brother, friend, boy, girl, family','head, eyes, ears, nose, mouth, hand, leg, foot','cat, dog, bird, fish, horse, rabbit, lion, bear','tree, flower, sun, moon, sky, leaf, bird, rain','school, teacher, book, pen, desk, class, bag, lesson','home, room, kitchen, bed, table, chair, door, window','apple, banana, bread, milk, water, rice, egg, juice','sunny, rainy, cloudy, windy, hot, cold, snow, umbrella','learn, speak, listen, read, write, help, try, great'];
const fallbackG1=(n)=>{const wi=Math.floor((n-1)/5)%12,t=G1Topics[wi],w=G1Words[wi],m=((n-1)%5)+1;const dialogue=m===2?'Hello! What is your name? — My name is Mia.':`Look at the ${w.split(', ')[(n+1)%8]}.`;const read=`This is Mission ${n}. We are learning about ${t}. We use simple English words, listen carefully, speak clearly, read a short text and practise.`;const q='What are we learning about?',a=t;const think=`Write or say one sentence about ${t.toLowerCase()}.`;return [t,w,dialogue,'My name is Mia.',read,q,a,think]};
const G1=G1Topics.map((topic,i)=>[topic,G1Words[i],'Hello! My name is Mia.','Nice to meet you!','This is a short English reading mission about '+topic+'.','What is the topic of this mission?',topic,'Tell me one thing you learned.']);
...
async function loadServerProgress(){
  ...
  try{
    ...
    if(!rewardError&&reward){
      if(Number.isFinite(Number(reward.xp)))state.serverXp=Number(reward.xp);
...
<section class="card activity"><h2>✍️ Writing / Practice</h2><p class="question">${esc(think)}</p><textarea class="answer" data-writing-answer placeholder="Write 2–4 English sentences..."></textarea><button class="btn" data-save-writing>💾 Save writing</button><div class="save-msg writing-save-msg"></div></section>
