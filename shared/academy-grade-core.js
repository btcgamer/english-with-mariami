...
async function loadDbLesson(n){
  ...
  try{
    const {data:lessonRow,error:lessonError}=await client
      .from('lessons')
      .select('id,lesson_number,title,topic,description')
      .eq('grade',grade)
      .eq('lesson_number',n)
      .maybeSingle();
    ...
    const result={...lessonRow,words:Array.isArray(wordRows)?wordRows:[]};
...
function missionData(n,L){
 ...
 if(m===2){const correct=reply.includes('—')?reply.split('—').pop().trim():reply.trim();const opts=shuffle([correct,'I do not know.','Maybe tomorrow.']);return {title:'Dialogue Lab',brief:'Listen to the conversation and choose the best reply.',html:`<p class="dialogue">${esc(dialogue)}</p><button class="btn" data-speak="${esc(dialogue)}">🔊 Listen</button><p class="question">What is the best response?</p><div class="choicegrid">${opts.map(x=>`<button class="btn choice" data-answer="${x===correct?'right':'wrong'}">${esc(x)}</button>`).join('')}</div><div class="quizmsg"></div>`};}
 if(m===3){const facts=[q,a,read].join(' '),statement=a.toLowerCase()==='mia'?'The child is named Mia.':`The text gives the answer: ${a}.`;const truth=facts.toLowerCase().includes(a.toLowerCase());return {title:'Reading Mission',brief:'Read carefully and prove that you understood the text.',html:`<p class="reading">${esc(read)}</p><button class="btn" data-speak="${esc(read)}">🔊 Listen to reading</button><p class="question">1. ${esc(q)}</p><div class="choicegrid">${shuffle([a,'The teacher','A different child']).map(x=>`<button class="btn choice" data-answer="${x===a?'right':'wrong'}">${esc(x)}</button>`).join('')}</div><p class="question">2. Is this statement true?</p><p class="example">“${esc(statement)}”</p><div class="choicegrid"><button class="btn choice" data-answer="${truth?'right':'wrong'}">True</button><button class="btn choice" data-answer="${truth?'wrong':'right'}">False</button></div><div class="quizmsg"></div>`};}
