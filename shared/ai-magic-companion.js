...
(function(){
  ...
  function purgeGradeLogout(){
    var path=location.pathname;
    if(!/\/grade[234](?:\/|$)/.test(path))return;
    var selectors=[
      '#ewmGradeLogout','#grade2Logout','#grade3Logout','#grade4Logout',
      '#grade2NavFix','.g3-logout','.g4-logout','a.nav-btn.logout',
  ...
  function boot(){
    purgeGradeLogout();
    if(document.body){
      new MutationObserver(function(){purgeGradeLogout();}).observe(document.body,{childList:true,subtree:true});
    }
  }
