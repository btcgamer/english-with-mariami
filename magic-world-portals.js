...
let refreshQueued=false;function refresh(){if(refreshQueued)return;refreshQueued=true;requestAnimationFrame(()=>{refreshQueued=false;if(!document.hidden&&!window.__mwpLaunching)build()})}function boot(){build();addEventListener('englishMariamiProgressUpdated',refresh,{passive:true});addEventListener('storage',refresh,{passive:true});}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();