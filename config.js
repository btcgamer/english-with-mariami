...
(function(){
      ...
      form.addEventListener('submit',async function(event){
        ...
        try{
          ...
          if(isTeacher){
            ...
            setTimeout(()=>location.replace('teacher-dashboard.html'),300); return;
          }
          if(role==='parent'){
            if(message){message.className='message success';message.textContent='👨‍👩‍👧 იხსნება სასწავლო აკადემია...';}
            setTimeout(()=>location.replace('academy.html'),300); return;
          }
          if(role==='student' && [2,3,4].includes(grade)){
            ...
            setTimeout(()=>location.replace('grade'+grade+'.html'),300); return;
          }
          ...
          if(message){message.className='message error';const text=String(error?.message||'');message.textContent=/invalid login credentials/i.test(text)?'❌ ელფოსტა ან პაროლი არასწორია.':'❌ '+(text||'შესვლა ვერ მოხერხდა.');}
  ...
  if(window.supabase&&typeof window.supabase.createClient==='function') init();
    ...
    else{const script=document.createElement('script');script.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';script.dataset.englishMariamiSupabase='1';script.onload=init;script.onerror=()=>console.error('Supabase JS failed to load; page remains visible.');document.head.appendChild(script);}
  }
})();
