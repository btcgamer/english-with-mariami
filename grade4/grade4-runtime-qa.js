/* Grade 4 — Runtime Interaction QA Harness v3
 ...
 * verifies the REAL completion flow advances state, and restores the learner's
 * original localStorage state at the end.
 */
(function(){'use strict';
const QA_PARAM='qa',STATE_KEY='magic-neon-grade-4',REPORT_KEY='grade4-runtime-qa-v3',SNAPSHOT_KEY='grade4-runtime-qa-snapshot-v3';
const TOTAL=60;
...
const readState=()=>{try{return JSON.parse(localStorage.getItem(STATE_KEY)||'{}')}catch(e){return {}}};
const answerFor=()=>[
...
function snapshot(){
  ...
  for(let i=0;i<localStorage.length;i++){
    ...
    if(k&&(/^(magic-neon-grade-4|grade4-answer-)/.test(k)))items[k]=localStorage.getItem(k);
  }
...
function restore(){
  ...
  for(let i=localStorage.length-1;i>=0;i--){
    const k=localStorage.key(i);
    if(k&&(/^(magic-neon-grade-4|grade4-answer-)/.test(k)))localStorage.removeItem(k);
  }
...
async function step(){
  if(mode==='done'){report=readJSON(REPORT_KEY,report);showResult();return;}
  snapshot();
  ...
  if(!complete||complete.disabled){writeJSON(REPORT_KEY,report);return}

  const before=readState();
  const beforeDone=new Set(Array.isArray(before.done)?before.done.map(Number):[]);
  complete.click();
  await sleep(250);

  // The harness must observe the application's own completion handler.
  // It must never write current=n+1 itself, otherwise the QA would produce a false pass.
  const after=readState();
  const afterDone=new Set(Array.isArray(after.done)?after.done.map(Number):[]);
  check(n,afterDone.has(n),`mission ${n} was not added to done[] by completion flow`);
  check(n,afterDone.size>=beforeDone.size,`done[] shrank from ${beforeDone.size} to ${afterDone.size}`);

  if(n<TOTAL){
    check(n,Number(after.current)===n+1,`real completion did not advance current: ${after.current}`);
    if(Number(after.current)!==n+1){writeJSON(REPORT_KEY,report);return}
    writeJSON(REPORT_KEY,report);
    const u=new URL(location.href);u.searchParams.set(QA_PARAM,'1');
    location.replace(u.href);
  }else{
    check(n,afterDone.size===TOTAL,`final done count=${afterDone.size}`);
    check(n,Number(after.current)===TOTAL||Number(after.current)===TOTAL+1,`final current=${after.current}`);
