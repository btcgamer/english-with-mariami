/* English with Mariami — Grade 4 runtime final audit
   ...
   It audits the actual lesson array immediately before the Grade 4 expansion layer runs.
*/
(function(){
  'use strict';
  ...
  function audit(lessons){
    ...
    if(list.length !== EXPECTED_LESSONS){
      issues.push(`Lesson count: expected ${EXPECTED_LESSONS}, found ${list.length}`);
    }
    ...
    list.forEach((lesson, index)=>{
      ...
      requiredText.forEach(key=>{
        if(!text(lesson[key])) issues.push(`${label}: missing ${key}`);
      });

      requiredArray.forEach(key=>{
        if(!Array.isArray(lesson[key])) issues.push(`${label}: ${key} is not an array`);
      });

      if(arr(lesson.words).length < 8) issues.push(`${label}: vocabulary below 8 words before expansion (${arr(lesson.words).length})`);
    ...
    for(let n=1;n<=EXPECTED_LESSONS;n++){
      if(!numbers.has(n)) issues.push(`Missing lesson_number ${n}`);
    }
  ...
  if(typeof previous === 'function'){
    window.applyGrade4ContentExpansion = function(lessons){
      const result = audit(lessons);
      window.ENGLISH_MARIAMI_G4_FINAL_AUDIT.lastResult = result;
      if(!result.ok) console.warn('[Grade 4 Final Audit]', result);
      else console.info('[Grade 4 Final Audit] PASS — 60/60 lesson structure is valid.', result);
