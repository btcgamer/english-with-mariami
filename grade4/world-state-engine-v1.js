...
const GRADE=4, KEY='grade4UniverseProgress', WORLD_SIZE=5;
let remote=null;
...
async function fetchRemote(){
 ...
 try{const {data:{user}}=await client.auth.getUser();if(!user)return null;const {data,error}=await client.from('lesson_progress').select('lesson_id,completed').eq('student_id',user.id).eq('grade',GRADE);if(error)throw error;return new Set((data||[]).filter(x=>x.completed).map(x=>{const m=String(x.lesson_id||'').match(/(?:lesson[-_ ]?)?(\d+)$/i);return m?Number(m[1])-1:-1}).filter(n=>n>=0&&n<60));}catch(e){console.warn('Grade 4 Supabase progress sync:',e);return null}}
...
window.addEventListener('storage',()=>sync());sync();window.G4WorldStateEngine={sync};
})();