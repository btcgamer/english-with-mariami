...
(function(){'use strict';
const GRADE=3,WORLD_SIZE=5,root=document.getElementById('missions');if(!root)return;
...
function openWorld(i){const card=root.querySelector(`[data-i="${i}"]`);if(!card)return;card.scrollIntoView({behavior:'smooth',block:'center'});const btn=card.querySelector('.g3-open');if(btn)setTimeout(()=>btn.click(),220)}
...
root.addEventListener('click',e=>{const card=e.target.closest('.mission');if(!card||card.dataset.worldState==='LOCKED')return;if(e.target.closest('.g3-open')){e.preventDefault();e.stopImmediatePropagation();routeWorld(Number(card.dataset.i)||0)}},true);
window.G3MissionRouterV2={nextLesson,routeStart,routeWorld};
})();
