...
function pointerFX(){if(reduced)return;let active=null;document.addEventListener('pointermove',e=>{if(innerWidth<760)return;const t=e.target.closest?.('.u-holo');if(!t)return;active=t;const r=t.getBoundingClientRect();const x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;const rx=(-y*7).toFixed(2),ry=(x*8).toFixed(2);t.style.setProperty('--u-mx',((x+.5)*100)+'%');t.style.setProperty('--u-my',((y+.5)*100)+'%');t.style.transform='perspective(1000px) rotateX('+rx+'deg) rotateY('+ry+'deg) translateY(-7px) translateZ(8px) scale(1.014)';},{passive:true});document.addEventListener('pointerout',e=>{const t=e.target.closest?.('.u-holo');if(t&&(!e.relatedTarget||!t.contains(e.relatedTarget))){t.style.transform='';t.style.removeProperty('--u-mx');t.style.removeProperty('--u-my');if(active===t)active=null;}},{passive:true})}
