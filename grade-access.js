...
(function(){
  'use strict';
  const m=(location.pathname||'').match(/(?:^|\/)grade([234])\.html$/i);
  if(!m)return;
  ...
  const target=g=>g===2?'grade2/index.html':'grade'+g+'.html';
  async function check(){
    ...
    try{
      ...
      if(role==='student'){
        ...
        if(current===2)return location.replace('grade2/index.html');
